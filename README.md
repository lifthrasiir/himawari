...
히마와리는 IRC 채널에 상주하면서 채널 사람들이 입력한 규칙에 따라 개드립을 쏟아내는 IRC 봇입니다. 한국어를 위해 설계되었지만 약간의 수정으로 다른 언어에도 대응 가능합니다. [특설 페이지](http://himawari.mearie.org/)도 참고하세요.

Himawari is an IRC bot designed to say rants, generated by the rules specific to and customized by given IRC channel. It is designed for Korean language but also adaptable to other languages with appropriate fixes. See also the [special page](http://himawari.mearie.org/) (Korean).
...
* 값 추가: `\<키>: <값>` 또는 `\<키>:: <값> <값> ...` 또는 `\<키>:/ <값> / <값> / ...`
	* 변경 명령과 혼동을 방지하기 위해 `<값>`에는 화살표가 들어갈 수 없습니다.
	* 여러 값을 추가하는 명령에서 `::`와 `:/` 사이에는 공백이 들어가면 안 됩니다.
	* 여러 값을 추가할 경우 구분자(공백이나 `/`)는 입력할 수 없습니다. 이 경우 하나씩 추가하는 명령을 써야 합니다.
